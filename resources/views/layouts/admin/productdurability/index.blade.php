<x-admin-layout>
  <div class="container px-6 mx-auto grid">
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
        ...
        <table class="w-full whitespace-no-wrap">
          <thead>
            <tr
              ...
              <th class="px-4 py-3 w-1/2">Ketahanan</th>
              <th class="px-4 py-3 w-3/12">Durasi Min</th>
          ...
          <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
            @foreach ($durabilities as $durability)
              <tr class="text-gray-700 dark:text-gray-400">
                <td class="px-4 py-3">
                  <div class="flex items-center text-sm cursor-pointer">
                    <div>
                      <p class="font-semibold">{{ $durability->nama }}</p>
                    </div>
                  </div>
                </td>
                ...
                <td class="px-4 py-3 text-sm flex gap-3">
                  <button
                    ...
                    id="delete-durability" data-id="{{ $durability->product_durability_id }}">
              ...
              <div class="fixed inset-0 z-30 hidden items-end bg-black bg-opacity-50 sm:items-center sm:justify-center"
                id="modal-delete-durability-{{ $durability->product_durability_id }}">
                <div
                  ...
                  <header class="flex justify-end">
                    <button
                      ...
                      aria-label="close" id="modal-close-button" data-id="{{ $durability->product_durability_id }}">
                  ...
                  <div class="mt-4 mb-6">
                    ...
                    <p class="text-sm text-gray-700 dark:text-gray-400">
                      Apakah Anda ingin menghapus ketahanan {{ $durability->nama }}?
                    </p>
                  </div>

                  <footer class="flex flex-col items-center justify-end gap-2 sm:flex-row bg-gray-50 dark:bg-gray-800">
                    <button id="modal-close-button" data-id="{{ $durability->product_durability_id }}"
                      class="w-full px-5 py-3 text-sm font-medium leading-5 text-gray-700 transition-colors duration-150 border border-gray-300 rounded-lg dark:text-gray-400 sm:px-4 sm:py-2 sm:w-auto active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray">
                      Batal
                    </button>

                    <form method="POST"
                      action={{ route('admin.durabilities.delete', ['id' => $durability->product_durability_id]) }}>
                      @csrf
                      @method('DELETE')
      ...
      {{ $durabilities->links('components.admin.pagination.pagination') }}
    </div>
  </div>

  <x-slot name="modal">
    <div class="fixed inset-0 z-30 hidden items-end bg-black bg-opacity-50 sm:items-center sm:justify-center"
      id="modal-add-durability">
      <div
        class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl"
        role="dialog" id="modal">
        ...
        <header class="flex justify-end">
          <button
            class="inline-flex items-center justify-center w-6 h-6 text-gray-400 transition-colors duration-150 rounded dark:hover:text-gray-200 hover: hover:text-gray-700"
            aria-label="close" id="add-durability-close">
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" role="img" aria-hidden="true">
              <path
                d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                clip-rule="evenodd" fill-rule="evenodd"></path>
        ...
        <div class="mt-4">
          ...
          <form method="POST" action={{ route('admin.durabilities.store') }}>
            @csrf
            ...
            <footer class="flex flex-col items-center justify-end gap-2 sm:flex-row bg-gray-50 dark:bg-gray-800 mt-6">
              <button id="add-durability-close" type="button"
                class="w-full px-5 py-3 text-sm font-medium leading-5 text-gray-700 transition-colors duration-150 border border-gray-300 rounded-lg dark:text-gray-400 sm:px-4 sm:py-2 sm:w-auto active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray">
                Batal
              </button>
  ...
  <x-slot name="script">
    ...
    <script type="text/javascript">
      $(document).ready(function() {
        $("body").on('click', "#delete-durability", function(e) {
          var durabilityId = $(this).data(("id"));
          $(`#modal-delete-durability-${durabilityId}`).removeClass('hidden').addClass('flex');
        });

        $("body").on('click', "#modal-close-button", function(e) {
          var durabilityId = $(this).data(("id"));
          $(`#modal-delete-durability-${durabilityId}`).removeClass('flex').addClass('hidden');
        });

        $("body").on('click', "#add-durability", function(e) {
          $(`#modal-add-durability`).removeClass('hidden').addClass('flex');
        });

        $("body").on('click', "#add-durability-close", function(e) {
          $(`#modal-add-durability`).removeClass('flex').addClass('hidden');
        });
      });
    </script>
  </x-slot>
